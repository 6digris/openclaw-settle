...
describe("createTelegramBot", () => {
  ...
  it.each([
    ...
    async ({ callbackId, callbackData, configuredModels, defaultModel, messageId, storeLabel }) => {
      ...
      expect(String(firstEditMessageTextArg(2))).toContain(
        "Session model selection cleared. Runtime set to <b>openclaw</b>. New replies use the agent's configured default.",
      );
      ...
      async ({
        ...
        if (outcomeText) {
          ...
          expect(confirmation).toBe(
            `${CHECK_MARK_EMOJI} Model reset to default\n\nSession model selection cleared. ${outcomeText} Runtime set to <b>${expectedRuntime}</b>. New replies use the agent's configured default.`,
          );
