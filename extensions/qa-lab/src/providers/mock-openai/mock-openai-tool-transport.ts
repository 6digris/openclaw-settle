...
export function readScenarioToolOutput(input: ResponsesInputItem[]) {
  ...
  // Only the matched dispatcher result owns the envelope; direct tool data is unchanged.
  if (
    result &&
    ...
    return extractToolOutput([{ type: "function_call_output", output: result.content }]);
  }
  return raw;
}
...
export function extractScenarioPlannedTool(events: StreamEvent[]) {
  ...
  if (wireName === "tool_call" && typeof wireArgs?.id === "string") {
    const args = wireArgs.args;
    return {
      name: wireArgs.id,
