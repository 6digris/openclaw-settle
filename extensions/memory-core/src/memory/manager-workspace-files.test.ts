...
describe("Gateway index over Harness workspace files", () => {
  ...
  it("refuses a registered remote workspace without Memory access instead of indexing stale files", async () => {
    await fs.writeFile(path.join(fixture.paths.workspace, "MEMORY.md"), "Gateway decoy.");
    ...
    expect(await getMemorySearchManager({ cfg, agentId: "main" })).toMatchObject({
      manager: null,
      error: expect.stringContaining("Remote Memory file access is unavailable"),
    });
  });
