...
email; `visitor_list` reads the policy again to reconcile that state. Revoke removes
policy access before deleting the record. Re-invite to retry an unsuccessful
invite, or revoke to clean up its record. Failure to read or qualify Gateway access
refuses an invitation before creating or renewing its grant.
