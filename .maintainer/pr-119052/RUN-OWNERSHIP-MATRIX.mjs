...
const [repo, root, head] = process.argv.slice(2);
...
const summary={head,driverSha256:createHash('sha256').update(await fs.readFile(new URL(import.meta.url))).digest('hex'),platform:process.platform,status:'RUNNING',cells:cases.map(id=>({id,status:'UNRUN'})),limitations:['Stale PID incarnation is a deliberately stale real SQLite lease bound to a live Windows PID; OS PID reuse is not forced.','No Scheduler action is registered by this ownership driver; maintained lifecycle proof covers that path.','Forced taskkill is observed natively with the production 15-second wait bound; an OS termination delay beyond five seconds is not synthesized.','Degraded CIM is one real PowerShell command forced to throw; tasklist output and all taskkill execution remain native.']};
    ...
    for(const item of identities){
      try{
        ...
        if(item.startedAt===null){
          // The ChildProcess owns its native handle even when CIM/creation-time lookup failed.
          ...
          assert.ok(item.child.exitCode!==null || item.child.signalCode!==null,'Spawned actor handle did not exit');
          assert.equal(probeProcessState(item.pid),'missing','Unverified actor PID remains; no PID-based kill attempted');
          continue;
        }
        const born=readWindowsProcessStartTimeSync(item.pid,5000,env);
        if(born!==null){assert.equal(born,item.startedAt,'Refuse cleanup of changed PID incarnation');
          const result=nativeSpawnSync(path.join(process.env.SystemRoot,'System32/taskkill.exe'),['/F','/T','/PID',String(item.pid)],{encoding:'utf8',timeout:15000});
          assert.equal(result.status,0,result.stderr);
        }
        assert.equal(probeProcessState(item.pid),'missing','Owned actor remains after cleanup');
