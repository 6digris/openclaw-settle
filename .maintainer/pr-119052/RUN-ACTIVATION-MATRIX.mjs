...
const [repo, proofRoot, expectedHead] = process.argv.slice(2);
...
for (const spec of cases) {
  ...
  const actorSource = `import fs from 'node:fs'; import path from 'node:path'; import net from 'node:net'; import {fileURLToPath} from 'node:url';
const root=path.dirname(fileURLToPath(import.meta.url)); const events=path.join(root,'events.jsonl');
const emit=(kind)=>fs.appendFileSync(events,JSON.stringify({kind,pid:process.pid,at:Date.now(),argv:process.argv.slice(2)})+'\\n');
...
`;
  ...
  function retainOwnedProcess(pid) {
    ...
    if (prior) assert.deepEqual(current, prior, "Owned PID incarnation changed");
    else ownedProcesses.set(pid, current);
  }
  ...
  try {
    ...
    for (const event of await nativeEvents()) retainOwnedProcess(event.pid);
    ...
    try {
      ...
      for (const [pid, expected] of ownedProcesses) {
        const current = identity(pid);
        if (!current) continue;
        assert.deepEqual(current, expected, "Refuse cleanup of a changed process incarnation");
