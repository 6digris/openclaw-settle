...
describe("scripts/lib/openclaw-e2e-instance.sh", () => {
  ...
  it("recovers when an offset-bearing gateway log is truncated before readiness", () => {
    withTempDir("openclaw-e2e-readyz-truncated-offset-", (tempDir) => {
      const logPath = path.join(tempDir, "gateway.log");
      fs.writeFileSync(logPath, "old log contents\n".repeat(100));
      const result = runBashWithHelper(
        [
          "openclaw_e2e_probe_http() { return 0; }",
          `scan_offset="$(wc -c <${shellQuote(logPath)})"`,
          ...
          `printf '[gateway] ready ws://127.0.0.1:23456\\n' >${shellQuote(logPath)}`,
          'child_status="unchanged"',
          `openclaw_e2e_wait_gateway_ready "$gateway_pid" ${shellQuote(logPath)} 2 23456 strict child_status "$((SECONDS + 2))" "$scan_offset"`,
          '[ -z "$child_status" ]',
        ],
        {},
        5_000,
  ...
  it("leaves child status empty when a live gateway reaches its deadline", () => {
    withTempDir("openclaw-e2e-readyz-deadline-", (tempDir) => {
      ...
      fs.writeFileSync(
        ...
        Array.from(
          { length: 140 },
          (_, index) => `diagnostic-line-${index.toString().padStart(3, "0")}\n`,
        ).join(""),
      );
      const result = runBashWithHelper(
        [
      ...
      expect(result.stdout).not.toContain("diagnostic-line-019");
      expect(result.stdout).toContain("diagnostic-line-020");
      expect(result.stdout).toContain("diagnostic-line-139");
    });
  });
