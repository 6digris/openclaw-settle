...
const FROZEN_COMPAT = resolve("scripts/lib/frozen-target-compat.sh");
const EXTENDED_WRAPPER = resolve("scripts/e2e/extended-stable-upgrade-survivor-docker.sh");
...
function runSurvivor(overrides: NodeJS.ProcessEnv = {}, shell = "bash") {
  ...
  writeExecutable(
    ...
    `#!/usr/bin/env bash
set -euo pipefail
if [[ "\${1:-}" == *package-openclaw-for-docker.mjs ]]; then
  printf '%s\n' "$GENERATED_PACKAGE_TARBALL"
  exit 0
fi
...
printf '%s' "\${OPENCLAW_CURRENT_PACKAGE_TGZ:-}" >"$CAPTURE_DIR/node-package"
...
`,
  ...
  const result = spawnSync(shell, [SCRIPT], {
    ...
    env: {
      ...
      GENERATED_PACKAGE_TARBALL: packageTarball,
...
describe("standalone upgrade survivor plugin registry", () => {
  ...
  it("forwards a package generated by the standalone wrapper into registry planning", () => {
    ...
    expect(result.status, result.stderr).toBe(0);
    expect(readFileSync(join(captureDir, "node-package"), "utf8")).toBe(packageTarball);
  });
...
describe("frozen upgrade survivor expectation ownership", () => {
  const contractNames = [
    "OPENCLAW_UPGRADE_SURVIVOR_EXEC_APPROVAL_OWNER",
    "OPENCLAW_UPGRADE_SURVIVOR_DISCORD_DM_OWNER",
    "OPENCLAW_UPGRADE_SURVIVOR_SESSION_METADATA_OWNER",
    "OPENCLAW_UPGRADE_SURVIVOR_LEGACY_RUNTIME_DEPS_OUTCOME",
    "OPENCLAW_UPGRADE_SURVIVOR_PREPUBLISH_PLUGIN_SOURCE",
    "OPENCLAW_UPDATE_FIXTURE_ALLOW_MISSING_LEGACY_COMPAT",
    "OPENCLAW_UPGRADE_SURVIVOR_DOCTOR_REPAIRED_SERVICE",
    "OPENCLAW_UPGRADE_SURVIVOR_MOCK_CONFIG_DIALECT",
  ];

  function resolveContract(version: string): string[] {
    const result = spawnSync(
      "bash",
      [
        "-c",
        `source "$1"; openclaw_apply_frozen_upgrade_survivor_contract "$2"; shift 2; for name in "$@"; do printf '%s=%s\\n' "$name" "\${!name-}"; done`,
        "contract",
        FROZEN_COMPAT,
        version,
        ...contractNames,
  ...
  it("applies legacy owners only to the exact frozen 7.33 candidate", () => {
    expect(resolveContract("2026.7.33")).toEqual([
      "OPENCLAW_UPGRADE_SURVIVOR_EXEC_APPROVAL_OWNER=json",
      "OPENCLAW_UPGRADE_SURVIVOR_DISCORD_DM_OWNER=legacy",
      "OPENCLAW_UPGRADE_SURVIVOR_SESSION_METADATA_OWNER=file",
      "OPENCLAW_UPGRADE_SURVIVOR_LEGACY_RUNTIME_DEPS_OUTCOME=removed",
    ...
    expect(resolveContract("2026.9.9")).toEqual(contractNames.map((name) => `${name}=`));
    const wrapper = readFileSync(EXTENDED_WRAPPER, "utf8");
    for (const name of contractNames) {
      expect(wrapper).not.toContain(`${name}=`);
    }
  });
});
