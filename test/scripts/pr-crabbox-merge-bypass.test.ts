...
if (args[0] === "repo" && args[1] === "view") out(args.includes("--jq") ? repo.nameWithOwner : repo);
  ...
  if (endpoint === "repos/openclaw/openclaw") {
    if (args[args.indexOf("--hostname")+1] !== "github.com") fail("missing repository hostname");
    out({id:repo.id,node_id:"fixture-repo",full_name:repo.nameWithOwner,html_url:repo.url});
  }
