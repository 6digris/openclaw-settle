...
openclaw_e2e_wait_gateway_ready() {
  ...
  local wait_ready_log_pattern='\[gateway\] ready'
  if [ "$wait_ready_mode" = "legacy-ready-log-ok" ]; then
    wait_ready_log_pattern='\[gateway\] (ready|listening on)'
  fi
  ...
  for ((attempt_index = 0; attempt_index < wait_ready_attempts; attempt_index++)); do
    ...
    if [ "$wait_ready_saw_log" != "true" ] && [ -f "$wait_ready_log" ]; then
      ...
      if [ "$log_bytes" -lt "$wait_ready_scan_offset" ]; then
        # Truncation starts a new log generation, including for offset-bearing callers.
        wait_ready_scan_offset=0
        wait_ready_scan_carry=""
      fi
      if [ "$log_bytes" -gt "$wait_ready_scan_offset" ]; then
        ...
        ready_log_lines="$(printf "%s" "$scan_text" | grep -E "$wait_ready_log_pattern" || true)"
    ...
    if [ "$wait_ready_saw_log" = "true" ]; then
      ...
      if [ "$wait_ready_mode" = "legacy-ready-log-ok" ]; then
        openclaw_e2e_probe_tcp 127.0.0.1 "$wait_ready_port" 400 && return 0
      else
