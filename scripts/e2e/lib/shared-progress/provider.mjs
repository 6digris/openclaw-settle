...
const required = (value, needle, stage) => {
  if (!value.includes(needle)) {
    throw new Error(`${stage}: actual tool result lacks ${needle}`);
  }
};
...
function reply(body, run, stage, name) {
  ...
  if (call.function?.name !== name) {
    throw new Error(`${stage}: tool name changed.`);
  }
  const result = body.messages.findLast(
    (message) => message.role === "tool" && message.tool_call_id === call.id,
  );
  if (!result) {
    throw new Error(`${stage}: no delivered result; refusing duplicate call.`);
  }
...
function exec(body, run, stage, title, command, seconds, commentary) {
  ...
  if (Object.hasOwn(schema(body, "exec").properties, "yieldMs")) {
    args.yieldMs = 120000;
  }
...
function deliveredFailures(body, run, accepted) {
  ...
  const interruption = (value, name) => {
    ...
    if (!execAbortReason(reason)) {
      throw new Error(`${name}: unsupported interruption report.`);
    }
  ...
  for (const message of body.messages) {
    ...
    for (const fragment of findings) {
      ...
      for (const section of fragment.matchAll(
        ...
        const name = children.find(
          (candidate) => candidate.toLowerCase() === section[1].toLowerCase(),
        );
        ...
        if (status) {
          save(name, status, "delivered-child-terminal-status");
        } else if (section[2] === "ok") {
          interruption(section[3], name);
        }
      }
    }
    ...
    for (const event of content.matchAll(
      ...
      const name = children.find(
        (candidate) => accepted.get(candidate)?.childSessionKey === event[1],
      );
      if (!name) {
        continue;
      }
...
function child(body, run, name) {
  ...
  if (preparationUnknown) {
    return preparationUnknown;
  }
  const preparationAbort = execAbortReason(preparation);
  if (preparationAbort) {
    return interruptedChild(run, name, `${name}-prepare`, preparationAbort);
  }
  if (!preparation.includes(marker)) {
    refuseChildExec(run, name, `${name}-prepare`, preparation);
  }
  ...
  if (commandAbort) {
    return interruptedChild(run, name, `${name}-hold`, commandAbort);
  }
  const result = resultPattern(run, name).exec(output);
  if (!result) {
    refuseChildExec(run, name, `${name}-hold`, output);
  }
...
function parent(body, run) {
  ...
  for (const name of children) {
    const spawnOutput = reply(body, run, `spawn-${name}`, "sessions_spawn");
    if (spawnOutput === undefined) {
      return tool(
        body,
        run,
        `spawn-${name}`,
    ...
    const receipt = JSON.parse(spawnOutput);
  ...
  if (yielded === undefined) {
    if (results.size) {
      throw new Error("A child completed before the required yield checkpoint.");
    }
    return tool(
      body,
      run,
  ...
  if (JSON.parse(yielded).status !== "yielded") {
    throw new Error("Parent did not actually yield.");
  }
  ...
  if (failures.size) {
    ...
    if (settled.length === children.length) {
      const interruptedMarker = `SHARED_PROGRESS_INTERRUPTED run=${run}`;
      if (
        body.messages.some(
          (message) =>
            message.role === "assistant" && text(message.content).includes(interruptedMarker),
        )
      ) {
  ...
  if (results.size === children.length) {
    const finalMarker = `SHARED_PROGRESS_FINAL run=${run}`;
    if (
      body.messages.some(
        (message) => message.role === "assistant" && text(message.content).includes(finalMarker),
      )
    ) {
  ...
  const names = [...results.keys()].toSorted((a, b) => (a < b ? -1 : a > b ? 1 : 0));
...
function decide(body) {
  if (!Array.isArray(body.messages)) {
    throw new Error("Use openai-completions messages.");
  }
  ...
  for (const input of inputs) {
    ...
    if (match) {
      return { run: match[1], actor: match[2], ...child(body, match[1], match[2]) };
    }
  }
  const request = inputs
    ...
    .find(Boolean);
  if (!request) {
    throw new Error("No synthetic scenario marker in actual request context.");
  }
  const run = request[1];
...
const server = http.createServer((request, response) => {
  void (async () => {
    if (request.method === "GET" && request.url === "/health") {
      writeJson(response, 200, { ok: true, fixture: "shared-progress-v1" });
      return;
    }
    ...
    /** @param {unknown} error */ (error) => {
      ...
      const fixtureToolResult =
        error instanceof Error && "fixtureToolResult" in error
          ? error.fixtureToolResult
          : undefined;
      const entry = {
        at: new Date().toISOString(),
        fixtureError: message,
        ...(fixtureToolResult ? { fixtureToolResult } : {}),
      };
      if (
        !writeRequestLogEntryOrFail(response, {
          requestLog: process.env.MOCK_REQUEST_LOG,
        ...
        writeJson(response, 400, {
          error: { message: `Shared-progress fixture refused to advance: ${message}` },
        });
      }
    },
  );
