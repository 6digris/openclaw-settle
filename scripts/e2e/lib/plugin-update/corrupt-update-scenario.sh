...
# The recovery registry adds the plugin; it must not replace the registry that
# supplied the candidate's unpublished core dependencies during the first update.
export OPENCLAW_NPM_REGISTRY_UPSTREAM="${NPM_CONFIG_REGISTRY:-${npm_config_registry:-${OPENCLAW_PREPUBLISH_PLUGIN_REGISTRY_URL:-https://registry.npmjs.org/}}}"
