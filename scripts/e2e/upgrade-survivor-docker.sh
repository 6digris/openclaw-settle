...
if [ "${OPENCLAW_UPGRADE_SURVIVOR_SESSION_GROUPS:-0}" = "1" ] && [ "$SCENARIO" != "legacy-operator-state" ]; then
  echo "Session group catalog proof requires legacy-operator-state." >&2
  exit 2
fi
...
if [ "${OPENCLAW_UPGRADE_SURVIVOR_PUBLISHED_BASELINE:-0}" = "1" ]; then
  ...
  docker_e2e_run_with_harness \
    ...
    -e OPENCLAW_UPGRADE_SURVIVOR_SESSION_GROUPS="${OPENCLAW_UPGRADE_SURVIVOR_SESSION_GROUPS:-0}" \
