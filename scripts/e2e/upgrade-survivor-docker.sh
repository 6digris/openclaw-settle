...
case "$context_status" in
  0)
    upgrade_target_identity="$(node --input-type=module - "$ROOT_DIR" "$OPENCLAW_SELECTED_SHA" "$HARNESS_ROOT_DIR/scripts/lib/release-version.mjs" <<'NODE'
...
process.stdout.write(`${train}|${version}`);
NODE
)"
    UPGRADE_TARGET_TRAIN="${upgrade_target_identity%%|*}"
    UPGRADE_TARGET_VERSION="${upgrade_target_identity#*|}"
    ;;
  1) ;;
  *) exit "$context_status" ;;
esac
openclaw_resolve_frozen_upgrade_survivor_capabilities "$ROOT_DIR"
openclaw_apply_frozen_upgrade_survivor_contract "$UPGRADE_TARGET_VERSION"
...
if [ "${OPENCLAW_UPGRADE_SURVIVOR_PUBLISHED_BASELINE:-0}" = "1" ]; then
  ...
  if [ "$CANDIDATE_IS_CURRENT" = "1" ] && [ "$SCENARIO" != "custom-plugin-siblings" ] && [ -z "${OPENCLAW_PREPUBLISH_PLUGIN_REGISTRY_DIR:-}" ]; then
    ...
    OPENCLAW_DOCKER_ALL_LANES="$registry_planner_lane" \
      ...
      OPENCLAW_CURRENT_PACKAGE_TGZ="$PACKAGE_TGZ" \
