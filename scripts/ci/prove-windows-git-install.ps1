...
param(
    ...
    [switch]$TraceReleasedDriverFilesystem
)
$ErrorActionPreference = 'Stop'
if ($TraceReleasedDriverFilesystem -and $ProofCase -ne 'published-driver') { throw 'Filesystem diagnostics require the isolated published-driver case.' }
...
$names = @('USERPROFILE', 'OPENCLAW_HOME', 'OPENCLAW_STATE_DIR', 'OPENCLAW_CONFIG_PATH', 'OPENCLAW_GIT_DIR', 'OPENCLAW_UPDATE_DEV_TARGET_REF', 'APPDATA', 'LOCALAPPDATA', 'NPM_CONFIG_PREFIX', 'npm_config_prefix', 'Path', 'TEMP', 'TMP', 'OPENCLAW_PROOF_TRACE_ENTRY', 'OPENCLAW_PROOF_TRACE_OUTPUT', 'OPENCLAW_PROOF_TRACE_ROOT')
$saved = @{}
...
$proof = [ordered]@{
    ...
    diagnosticOnly = [bool]$TraceReleasedDriverFilesystem
...
try {
    ...
    if ($TraceReleasedDriverFilesystem) {
        $env:OPENCLAW_PROOF_TRACE_ENTRY = $driver
        $env:OPENCLAW_PROOF_TRACE_ROOT = $prefix
        $env:OPENCLAW_PROOF_TRACE_OUTPUT = Join-Path $EvidenceRoot 'released-driver-filesystem'
        $driverArgs = @('--require', (Join-Path $CandidateRoot 'scripts/ci/trace-released-driver-filesystem.cjs')) + $driverArgs
    }
    try {
        Invoke-ProofCommand -Name 'published-driver-update' -File $node -Arguments $driverArgs -Seconds 3600
    } finally {
        foreach ($traceName in @('OPENCLAW_PROOF_TRACE_ENTRY', 'OPENCLAW_PROOF_TRACE_OUTPUT', 'OPENCLAW_PROOF_TRACE_ROOT')) {
