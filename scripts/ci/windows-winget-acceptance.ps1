...
$proof = [ordered]@{ scenario=$Scenario; result='unqualified'; cleanup='not-started'; candidate=$ExpectedHead; prerequisiteOnly=[bool]$PrerequisiteOnly; candidateExecuted=$false; commands=@(); failures=@() }
$originalPaths = @{}
$breakpoints = @()
$ownedProduct = $null
$portableOwned = $false
...
function Initialize-PublicPortableNode {
    ...
    $script:runtime = $executables[0].FullName
}
try {
    ...
    $proof.host.storage = @()
    $destinations = @($ProofRoot,$WorkRoot,$env:ProgramFiles,$env:TEMP)
    if (-not $PrerequisiteOnly) { $destinations += $CandidateRoot }
    ...
    # Use a checksum-pinned official module and release, never the image client.
    $bootstrapRoot = Join-Path $WorkRoot 'winget-bootstrap'
    New-Item -ItemType Directory -Path $bootstrapRoot | Out-Null
    $moduleArchive = Join-Path $bootstrapRoot 'Microsoft.WinGet.Client.1.29.280.zip'
    $moduleUrl = 'https://www.powershellgallery.com/api/v2/package/Microsoft.WinGet.Client/1.29.280'
    Invoke-WebRequest $moduleUrl -OutFile $moduleArchive
    $moduleHash = (Get-FileHash $moduleArchive -Algorithm SHA512).Hash
    $expectedModuleHash = [Convert]::ToHexString([Convert]::FromBase64String('3lgY2mTUNikE/+NXN6LX38cXnLTSSOWBbOia/n4IwKisYwEvVIkJX0lyThvsDqouU6kFk67yBJed5ssnU0qRpg=='))
    Assert-Proof ($moduleHash -ceq $expectedModuleHash) 'Official WinGet module archive hash mismatch.'
    $moduleRoot = Join-Path $bootstrapRoot 'Microsoft.WinGet.Client'
    Expand-Archive $moduleArchive -DestinationPath $moduleRoot
    $module = Test-ModuleManifest (Join-Path $moduleRoot 'Microsoft.WinGet.Client.psd1')
    Assert-Proof ($module.Version.ToString() -ceq '1.29.280') 'Unexpected WinGet bootstrap module version.'
    $bundleUrl = 'https://github.com/microsoft/winget-cli/releases/download/v1.29.290/Microsoft.DesktopAppInstaller_8wekyb3d8bbwe.msixbundle'
    $bundle = Join-Path $bootstrapRoot 'DesktopAppInstaller.msixbundle'
    Invoke-WebRequest $bundleUrl -OutFile $bundle
    $bundleHash = (Get-FileHash $bundle -Algorithm SHA256).Hash
    Assert-Proof ($bundleHash -ceq '6824B6E9484AB24687D99A0C829D2BCBCC7849A70A4D0F596FC43C89D20DFF15') 'Official WinGet release bundle hash mismatch.'
    # Bind the registered executable to the exact x64 payload in the fixed release.
    $bundleZip = [IO.Compression.ZipFile]::OpenRead($bundle)
    try {
    ...
    } finally { $bundleZip.Dispose() }
    $applicationZip = [IO.Compression.ZipFile]::OpenRead($applicationPath)
    try {
        $officialExe = Join-Path $bootstrapRoot 'winget.exe'
        [IO.Compression.ZipFileExtensions]::ExtractToFile($applicationZip.GetEntry('winget.exe'),$officialExe)
        $reader = [IO.StreamReader]::new($applicationZip.GetEntry('AppxManifest.xml').Open())
        try { [xml]$applicationManifest = $reader.ReadToEnd() } finally { $reader.Dispose() }
    } finally { $applicationZip.Dispose() }
...
'@ | Set-Content $bootstrapScript
    ...
    Assert-Proof ($packages[0].Version.ToString() -ceq $applicationManifest.Package.Identity.Version -and $packages[0].Publisher -ceq $applicationManifest.Package.Identity.Publisher) 'Registered package differs from the fixed official release.'
    Assert-Proof ((Get-FileHash $wingetPayload).Hash -ceq $proof.bootstrap.payloadSha256) 'Registered Winget executable differs from pinned official payload.'
    $registeredSignature = Get-AuthenticodeSignature -LiteralPath $wingetPayload
    Assert-Proof ($registeredSignature.Status -eq 'Valid' -and $registeredSignature.SignerCertificate.Thumbprint -ceq $proof.bootstrap.signerThumbprint) 'Registered payload signature differs.'
    ...
    if ($Scenario -eq 'non-msi') { Initialize-PublicPortableNode }
    if ($PrerequisiteOnly) {
        $proof.before = Get-RuntimeFacts $runtime 'runtime-before'
        Assert-Proof ($proof.before.version -ceq 'v24.19.0') 'Acquired portable runtime version differs.'
        $proof.outcome = 'portable-prerequisite-only'
    } else {
        $proof.candidateExecuted = $true
        ...
        if ($Scenario -eq 'unsupported-node') {
            ...
            if ($Scenario -eq 'non-msi') {
                # Shared portable setup completed before any candidate code.
            ...
            $proof.registration = @(Get-NodeRegistration)
            if ($Scenario -eq 'non-msi') {
                # Shared portable setup already verified native registration and exact-ID correlation.
            } else {
