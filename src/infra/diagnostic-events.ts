...
import {
  consumeCoreGatewayAdmissionEvent,
  consumeCoreGatewayOwnerEvent,
} from "./diagnostic-gateway-admission-provenance.js";
import type {
  DiagnosticGatewayRpcEvent,
  DiagnosticGatewayAdmissionEvent,
  DiagnosticGatewayRunOwnerEvent,
} from "./diagnostic-gateway-events.types.js";
import {
...
export type DiagnosticEventMetadata = Readonly<{
  ...
  trusted: boolean;
  /** Exact core-producer provenance; never authority or a payload-supplied claim. */
  coreGatewayAdmission?: true;
...
type EmitDiagnosticEventOptions = {
  ...
  coreGatewayAdmission?: boolean;
  coreGatewayOwner?: boolean;
  hostPluginId?: string;
  internal?: boolean;
  privateData?: DiagnosticEventPrivateData;
  trustedTraceContext?: boolean;
...
function emitDiagnosticEventWithTrust(
  ...
  const metadata: InternalDiagnosticEventMetadata = {
    ...
    ...(options.coreSemanticRunProgress === true
      ...
      : {}),
    ...(options.coreGatewayAdmission === true ? { coreGatewayAdmission: true } : {}),
...
export function emitTrustedDiagnosticEvent(event: DiagnosticEventInput) {
  ...
  const coreGatewayAdmission = consumeCoreGatewayAdmissionEvent(event);
  const coreGatewayOwner = consumeCoreGatewayOwnerEvent(event);
  emitDiagnosticEventWithTrust(event, true, {
    ...(coreGatewayAdmission ? { coreGatewayAdmission: true, internal: true } : {}),
    ...(coreGatewayOwner ? { coreGatewayOwner: true, internal: true } : {}),
