...
(async () => {
  ...
  try {
    ...
    const previousGeneration = (restorationArmed || foregroundParked) && recoveryRun?.status === "running" &&
    ...
    if (params.foregroundOrigin) {
      ...
      // A pending sibling does not revoke the installed target or keep the closed origin down.
      // The canonical child owns this classification; its shared outcome remains skipped.
