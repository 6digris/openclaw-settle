...
import {
  invalidatePreparingPluginGenerations,
...
export async function completeUpdateCandidatePluginRehearsal(params: {
  ...
  return withPluginGenerationRehearsalPreparation(
    {
      env: params.env,
    ...
    async () => {
      ...
      const copiedGraph = inspectPluginSourceDependencies(entries);
      ...
      for (const entry of entries) {
        const unresolvedPrivate = copiedGraph.byEntry
          .get(entry.entryFile)!
          .unresolved.filter(
            ({ source, specifier }) =>
              isPathInside(privateRoot, source) && isPrivateLookup(specifier),
          );
        ...
        try {
          ...
          warnings.push(
            `Update checks could not inspect the original plugin source: ${entryFile}.`,
          );
          continue;
        }
        if (
        ...
        if (canonicalSource !== entryFile || (await fs.realpath(rootDir)) !== rootDir) {
          throw new Error(
            `Plugin source location changed since the update snapshot: ${entryFile}.`,
          );
        }
        for (const [source, copied] of [
          ...
          [
            ...
            path.join(copiedRoot, "openclaw.plugin.json"),
          ],
        ] as const) {
