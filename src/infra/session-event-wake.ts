...
import {
  ...
  getPublishedSystemEventStorePath,
...
function createSessionEventWakeRuntime() {
  ...
  function retire(wake: PendingWake): void {
    if (wake.retired) {
      return;
    }
    wake.retired = true;
  ...
  registerSystemEventStoreOwner(Symbol.for("openclaw.sessionEventWake"), () => {
    for (const [key, group] of pending) {
      for (const slot of SLOTS) {
        ...
        if (wake && !isSystemEventStoreCurrent(wake, wake.sessionStorePath)) {
          delete group[slot];
          retire(wake);
        }
      }
    ...
    for (const owner of active.values()) {
      ...
      for (const wake of owner.wakes) {
        if (!isSystemEventStoreCurrent(wake, wake.sessionStorePath)) {
          retire(wake);
          replaced = true;
        }
      }
      if (replaced) {
        owner.controller.abort(new Error("Session store replaced"));
      }
    }
    clearTimeout(timer);
    timer = undefined;
  ...
  function enqueue(wake: PendingWake, blockedUntil = 0): string {
    ...
    if (wake.retired || !isSystemEventStoreCurrent(wake, wake.sessionStorePath)) {
      retire(wake);
      return key;
    }
        ...
        sessionStorePath:
          options.sessionStorePath === undefined
            ? normalized.sessionKey
              ? getPublishedSystemEventStorePath(normalized)
              : undefined
