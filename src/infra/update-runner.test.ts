...
import {
  ...
  writePreflightPackageManagerFixture,
...
describe("updateGitCheckout", () => {
  ...
  type TestCommandOptions = import("../process/exec.js").CommandOptions;
    ...
    return async (argv: string[], options?: TestCommandOptions): Promise<CommandResult> => {
      ...
      if (executable === "git" && root) {
        ...
        if (command === "index-pack") {
          ...
          assert(typeof descriptor === "number");
          expect(readFileSync(descriptor, "utf8")).toBe("synthetic Git transport pack\n");
  ...
  function fixtureAdmission(
    ...
    return {
      ...
      validateCandidate: async (candidateRoot: string) => {
        expect(candidateRoot).not.toBe(root);
        expect(await pathExists(path.join(candidateRoot, "dist", "entry.js"))).toBe(true);
        validated = true;
      },
