...
describe("session wake physical store ownership", () => {
  it.each([false, true])(
    "does not adopt targeted work without an accepted store (captured: %s)",
    async (captured) => {
      await withOpenClawTestState({ scenario: "minimal" }, async ({ stateDir }) => {
        await drainGlobalSingletonLifecycleState();
        clearRuntimeConfigSnapshot();
        vi.useFakeTimers();
        const store = path.join(stateDir, "parent.sqlite");
        const handler = vi.fn(async () => ({ status: "ran" as const, durationMs: 1 }));
        dispose = setSessionEventWakeHandler(handler);
        const sessionStorePath = captured ? store : undefined;
        enqueueSystemEvent("unowned child changed", {
