...
import { detectGatewayRespawnSupervisor } from "../infra/supervisor-markers.js";
...
type RuntimeBuildGeneration = { buildId: string };
...
async function isRuntimeActivationManual(installRoot: string): Promise<boolean> {
  try {
    await access(path.join(installRoot, RUNTIME_ACTIVATION_MANUAL_RELATIVE_PATH));
    return true;
  } catch (error) {
...
export function startGatewayRuntimeGenerationMonitor(params: {
  ...
  isActivationManual?: (installRoot: string) => Promise<boolean>;
  canScheduleAutomaticRestart?: () => boolean;
  ...
  const isActivationManual = params.isActivationManual ?? isRuntimeActivationManual;
  const canScheduleAutomaticRestart =
    params.canScheduleAutomaticRestart ??
    (() => !isContainerEnvironment() || detectGatewayRespawnSupervisor(process.env) !== null);
  ...
  const check = async () => {
    ...
    const activationManual = await isActivationManual(installRoot);
    if (stopped || restartScheduled) {
      observedBuildId = null;
      return;
    }
    ...
    if (!canScheduleAutomaticRestart()) {
      restartScheduled = true;
      clearInterval(timer);
      params.log.warn(
        `runtime generation ${generation.buildId} requires an external container restart`,
      );
      return;
    }
