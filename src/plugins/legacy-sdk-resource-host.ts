...
export class LegacyPluginSdkResourceHost {
  ...
  invokeDetached(run: () => void | Promise<void>, reportError: (error: unknown) => void): void {
    this.assertOpen();
    const invocationInstance = pluginInstanceInvocation.getStore()?.instance;
    const instance = invocationInstance ? getPluginValueInstance(invocationInstance) : undefined;
    if (
      invocationInstance &&
      (!instance ||
        instance !== invocationInstance ||
        !instance.acceptingCalls ||
        getPluginInstanceOwner(instance)?.revoked)
    ) {
      throw new PluginInstanceUnavailableError(invocationInstance.pluginId);
    }
