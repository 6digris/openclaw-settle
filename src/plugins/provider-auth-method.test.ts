...
describe("runProviderPluginAuthMethodUnpersisted", () => {
  ...
  it.each([false, true])(
    "keeps device destinations with their codes and cancellation (remote=%s)",
    async (isRemote) => {
      const method: ProviderAuthMethod = {
        ...
        kind: "device_code",
        run: async (ctx) => {
          if (!ctx.prompter.deviceCode || !ctx.signal) {
            throw new Error("Expected a cancellable device-code wizard");
          }
          await ctx.openUrl(destination);
          ...
          await delay(5_000, undefined, { signal: ctx.signal });
          return { profiles: [] };
        },
      };
