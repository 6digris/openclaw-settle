...
const nativeExecutorUrl = resolveRuntimeWorkerUrl(updateExecutorNativeEntrypoints.nativeExecutor);
const sourceLoader = nativeExecutorUrl.pathname.endsWith(".ts")
...
it.each([
  ...
  async ({ supported, destination }) => {
    ...
    const {runGatewayServiceUpdateCommand}=await import(${JSON.stringify(nativeExecutorUrl.href)});
...
it.each(["receiver-root", "original-lineage", "stripped-lineage"] as const)(
  ...
  async (substitution) => {
    ...
    const receiver = `
    import {runGatewayServiceUpdateCommand} from ${JSON.stringify(nativeExecutorUrl.href)};
