import "./run.option-collisions.test-harness.js";
...
import { captureManagedConfigSnapshotPreparation } from "../../config/runtime-snapshot.js";
...
const {
  ...
  gatewayLogMessages,
  gatewayStartOptions,
  installGatewayRunOptionCollisionFixture,
...
} = await import("./run.option-collisions.test-harness.js");
...
describe("gateway run option collisions", () => {
  ...
  it("starts gateway when token mode has no configured token (startup bootstrap path)", async () => {
    const preparations: ReturnType<typeof captureManagedConfigSnapshotPreparation>[] = [];
    readConfigFileSnapshotWithPluginMetadata.mockImplementationOnce(async () => {
      const preparation = captureManagedConfigSnapshotPreparation(
        "/tmp/openclaw-test-missing-config.json",
      );
      preparations.push(preparation);
      expect(captureManagedConfigSnapshotPreparation("/tmp/other-openclaw.json")).toBeNull();
      if (preparation) {
        expect(await preparation(async (prepare) => prepare)).toBe(prepareHostConfigSnapshot);
      }
      return { snapshot: configState.snapshot };
    });
    ...
    expect(preparations).toEqual([expect.any(Function)]);
    const preparation = preparations[0];
    if (!preparation) {
      throw new Error("The final Gateway config read did not receive host preparation");
    }
    const afterRead = vi.fn(async () => undefined);
    await expect(preparation(afterRead)).rejects.toThrow("snapshot preparation owner has closed");
