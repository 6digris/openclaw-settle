...
import { INHERITED_PROCESS_LINEAGE_FD_ENV } from "./inherited-process-lineage.js";
...
export function runServiceChildGroupAnchor(): void {
  ...
  const requestCleanup = async (
    ...
    if (start.ownedWorker) {
      ...
      // The application may die before its detached command owners. The journaled
  ...
  const onControlMessage = (message: ServiceChildControlMessage) => {
    ...
    if (message.type === "worker-start") {
      ...
      workerStarted = true;
      command.send({ type: "openclaw-worker-start-v1" }, (error) => {
        if (error) {
          void requestCleanup("parent-lost");
        }
      });
  ...
  const startCommand = async (next: ServiceChildStart) => {
    ...
    try {
      command = spawn(start.command, start.args, {
        cwd: start.cwd,
        env: start.ownedWorker
          ? { ...start.env, [INHERITED_PROCESS_LINEAGE_FD_ENV]: inheritedLineageFds.join(",") }
          : start.env,
        argv0: start.argv0,
        stdio,
        detached: false,
