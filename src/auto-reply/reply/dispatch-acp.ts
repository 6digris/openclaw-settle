...
import { assertPreparedAcpRouteCurrent } from "./dispatch-from-config.context.js";
...
export async function tryDispatchAcpReplyCore(params: {
  ...
  try {
    if (
      ...
      (await claimPreparedPendingAgentQuestionAnswer(
        ...
        () => assertPreparedAcpRouteCurrent(params.ctx),
      ))
    ) {
  ...
  try {
    ...
    await persistInput?.();
    await assertPreparedAcpRouteCurrent(params.ctx);
    assertInputCurrent();
