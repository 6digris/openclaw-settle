...
export function createReplyMediaSourcePreparer(params: {
  ...
  const normalizeMediaSource = async (
    ...
    const isRelativeLocalMedia =
      ...
      !WINDOWS_DRIVE_RE.test(media);
    // A remote workspace reader owns relative output paths. Resolving them via
    // a Gateway sandbox would select a different filesystem (and possibly stale bytes).
    const sandboxWorkspace =
      isRelativeLocalMedia && params.workspaceMediaRoot && params.workspaceMediaAccess?.readFile
        ? undefined
        : await resolveSandboxWorkspace();
    if (sandboxWorkspace) {
