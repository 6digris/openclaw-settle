...
describe.runIf(process.platform === "linux")(
  "workspace attachment adapter with the real remote-shell bridge",
  () => {
    it.each(["inbound", "remote-cache"])(
      ...
      async (directory) => {
        ...
        const ctx = { media };
        const release = registerAgentWorkspaceAccess(f.gateway, { bridge: f.bridge });
        try {
    ...
    it.each(["unmarked-directory", "bad-marker", "non-file"])(
      ...
      async (kind) => {
        ...
        if (kind === "bad-marker") {
          expect(
            await fs.readFile(path.join(path.dirname(destination), ".gitignore"), "utf8"),
          ).toBe("user marker");
        }
      },
    );
