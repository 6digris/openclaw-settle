...
import { registerStartupExhaustionRecoveryCases } from "./main-session-startup-exhaustion.test-harness.js";
...
let stopStartupExhaustionRecovery: (() => Promise<void>) | undefined;
...
afterEach(async () => {
  try {
    // A runner timeout need not unwind the test body before fixture teardown.
    await stopStartupExhaustionRecovery?.();
  } finally {
    resetGatewayWorkAdmission();
...
describe("main-session-restart-recovery", () => {
  ...
  stopStartupExhaustionRecovery = registerStartupExhaustionRecoveryCases(() => ({
    tmpDir,
    gatewayRuntime: mockRecoveryRuntime,
    loadSessionEntry,
