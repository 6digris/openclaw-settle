...
export const SANDBOX_PINNED_MUTATION_PYTHON = [
  ...
  "            raise ValueError('invalid directory entry limit')",
  "        try:",
  ...
  "                        raise OSError(errno.EFBIG, 'directory exceeds entry limit')",
  "                    kind = 'file' if entry.is_file(follow_symlinks=False) else 'directory' if entry.is_dir(follow_symlinks=False) else 'other'",
  "                    entries.append({'name': entry.name, 'type': kind})",
  "            output = json.dumps(entries)",
  "            if len(output) > 4194304:",
  "                raise OSError(errno.EFBIG, 'directory exceeds output limit')",
  "            print(output)",
  "    finally:",
