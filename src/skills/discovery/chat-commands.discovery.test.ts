...
describe("skill command discovery through workspace loading", () => {
  ...
  it("loads the remote bundled command despite a workspace collision and Gateway-only binaries", async () => {
    const root = tempDirs.make("remote-bundled-command-");
    const gateway = path.join(root, "gateway");
    const remote = path.join(root, "remote");
    const bundled = path.join(remote, "bundled");
    await writeSkill({
      dir: path.join(bundled, "control-ui"),
      name: "control-ui",
      description: "Remote bundled dashboard",
    ...
    try {
      ...
      const loaders = createSkillCommandLoaders(() => import("./chat-commands.runtime.js"), params);
      expect(await loaders.loadSkillCommands!()).toEqual([
        expect.objectContaining({ skillName: "control-ui", description: "Workspace replacement" }),
      ]);
