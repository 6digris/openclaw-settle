...
export async function prepareSkillResourceDelivery(
  ...
  const getSourceReader = (skill?: Skill) => {
    ...
    if (!sourceReader) {
      ...
      const access = sourceWorkspace ? getAgentWorkspaceAccess(sourceWorkspace) : undefined;
      if (access && !access.skillResources) {
        throw new WorkspaceAccessUnavailableError(
          "Remote workspace skill resources are unavailable",
        );
      }
      sourceReader = access?.skillResources ?? localSkillResourceReader;
    }
    return sourceReader;
  };
