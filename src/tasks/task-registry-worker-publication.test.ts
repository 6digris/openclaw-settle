...
describe("worker publication scope", () => {
  ...
  it("retains an absent predecessor baseline until a broad-scope receipt arrives", async () => {
    const { store, context, events } = await prepare([task]);
    const firstStarted = createDeferred();
    const firstRelease = createDeferred();
    const secondWritten = createDeferred();
    const secondReceipt = createDeferred();
    const first = runTaskRegistryWorkerMutation(
      {
        admission: context.admission,
        scope: { taskId: task.taskId },
        publicationRecords: () => new Map([[task.taskId, undefined]]),
        beforeObservers: async () => {
          firstStarted.resolve();
          await firstRelease.promise;
        },
      },
      async () => store.deleteTaskWithDeliveryState(task.taskId),
      async () => store.loadSnapshot(),
    );
    ...
    const second = runTaskRegistryWorkerMutation(
      {
        admission: context.admission,
        scope: { taskId: "provisional-id", runId: task.runId },
        publicationRecords: () => new Map([[task.taskId, undefined]]),
      },
      async () => {
        const transient = { ...task, task: "Unannounced replacement" };
        store.upsertTaskWithDeliveryState({ task: transient });
        publishTaskRecordAfterAtomicStore(transient, { deferredObserverEvents: [] });
    ...
    try {
      ...
      await second;
      expect(events).toEqual(["deleted:existing-task:original-run"]);
    } finally {
      firstRelease.resolve();
      secondReceipt.resolve();
      await Promise.allSettled([first, second]);
  ...
  it.each(["existing row", "prior absence"] as const)(
    "inherits the actual %s baseline before a superseded predecessor retires",
    async (initial) => {
      const { store, context, events } = await prepare(initial === "existing row" ? [task] : []);
      const firstRow = { ...task, runId: "unannounced-run" };
      const firstStarted = createDeferred();
      ...
      try {
        ...
        await second;
        expect(events).toEqual(
          initial === "existing row" ? ["deleted:existing-task:original-run"] : [],
        );
      } finally {
        firstRelease.resolve();
        secondRelease.resolve();
  ...
  it.each(["rebound", "rebound then deleted", "deleted"] as const)(
      ...
      await runTaskRegistryWorkerMutation(
        {
          ...
          publicationRecords: () =>
            new Map([
              [task.taskId, change === "deleted" ? undefined : { ...task, task: "Committed" }],
            ]),
        },
        async () => {
          if (change === "deleted") {
...
describe("worker publication during canonical reads", () => {
  ...
  it.each(["value", "absent"] as const)(
    ...
    async (change) => {
      ...
      const store = configureTaskSnapshot(change === "value" ? [task] : []);
      const read = captureTaskPageRead();
      await readTaskPage({ offset: 0, limit: 1 }, read);
      const published: string[] = [];
      configureTaskRegistryRuntime({
  ...
  it.each(["unrelated read", "same-task read", "before observers"] as const)(
    ...
    async (change) => {
      ...
      const store = configureTaskSnapshot([task, { ...task, taskId: "unrelated-task" }]);
      const read = captureTaskPageRead();
      await readTaskPage({ offset: 0, limit: 1 }, read);
      const published: string[] = [];
      configureTaskRegistryRuntime({
