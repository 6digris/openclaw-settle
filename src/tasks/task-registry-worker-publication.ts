...
}): Promise<{ snapshot: TaskRegistryStoreSnapshot; conflicted: boolean }> {
  ...
  try {
    ...
    for (const [taskId, expected] of pending.publication?.records ?? []) {
      ...
      if (
        expected === undefined
          ? current === undefined
          : current !== undefined && isEquivalentTaskRecord(expected, current)
      ) {
        pending.publication?.ready.add(taskId);
      }
    }
    return { snapshot, conflicted: merged.conflicted };
  } finally {
    delete pending.readWitness;
...
export function publishTaskRegistryWorkerMutation(params: {
  ...
  for (const [taskId, expected] of publication.records) {
    ...
    if (
      expected === undefined
        ? next !== undefined
        : next === undefined || !isEquivalentTaskRecord(expected, next)
    ) {
      continue;
    }
    const previous = pending.published.get(taskId);
    if (
      !isDeepStrictEqual(previous, next && cloneTaskRecordForObserver(next)) ||
      (forced?.taskId === taskId && next && isEquivalentTaskRecord(forced, next))
    ) {
      if (next) {
        emit(() => ({
          kind: "upserted",
        ...
        emit(() => ({ kind: "deleted", taskId, previous }));
      }
    }
  }
}
...
export function claimTaskRegistryPublication(
  pending: PendingTaskRegistryMutation,
  records: ReadonlyMap<string, TaskRecord | undefined>,
): void {
  ...
  pending.publication = {
    records: new Map(
      Array.from(records, ([taskId, record]) => [taskId, record && cloneTaskRecord(record)]),
    ),
    ready: new Set(),
    invalidated: new Set(),
  };
    ...
    for (const other of getTaskRegistryProcessState().projection.pending) {
      if (
        ...
        (other.scope.taskId === taskId ||
          (record && matchesScope(record, other.scope)) ||
