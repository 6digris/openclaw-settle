  ...
  const settled = await runTaskRegistryWorkerMutation(
    {
      ...
      publicationRecords: () =>
        new Map<string, TaskRecord | undefined>(
          committed ? [[committed.task.taskId, committed.task]] : [],
        ),
      beforeObservers: async () => {
        flowHookEntered = true;
        if (committed) {
...
async function finishTaskMutation(
  ...
  await syncFlowFromTaskAfterTaskMutationAsync(context, store, task, options.operation);
  if (options.operation === "update") {
    try {
      ...
      retainTaskMutationFlowEffects(context, store, flowStore, task, "update");
    }
  }
}
...
function retainTaskMutationFlowEffects(
  ...
  const assertFlowStore = () => {
    if (getTaskFlowRegistryStore() !== flowStore) {
      throw new Error("Committed task flow store is no longer current");
    }
  };
  try {
    const owner = taskFlowSyncOwner(task.taskId);
    retainCommittedTaskFlowEffects(
      ...
      {
        ...owner,
        assertCurrent(retryContext, retryStore) {
          owner.assertCurrent(retryContext, retryStore);
          assertFlowStore();
        },
      },
      operation === "update"
        ? (retryContext) =>
            finishManagedTaskCancellation(retryContext, store, flowStore, task.taskId, () => {
              assertTaskRegistryOwnerCurrent(retryContext, store);
              assertFlowStore();
            })
        : undefined,
    );
