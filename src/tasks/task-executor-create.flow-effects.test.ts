...
import { fixture, flow, ownerKey } from "./task-executor-create.flow-effects.test-support.js";
import { finishTaskMutation } from "./task-executor-mutation-effects.async.js";
...
it("keeps task identity readable while cancellation bookkeeping checks a mirrored flow", async () => {
  ...
  const created = expectDefined(await f.create(), "Expected the linked task");
  const read = expectDefined(await prepareTaskBackingRead(), "Expected a prepared task reader");
