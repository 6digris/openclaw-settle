        ...
        with self.lock:
            ...
            if method == "connect":
                return {
                    ...
                    "features": {"methods": [
                        "agents.list", "config.get", "chat.send", "chat.history",
                        "tasks.list", "progressCard.get",
                    ]},
            ...
            if method in ("tasks.list", "progressCard.get"):
                if not any(send["sessionKey"] == params.get("sessionKey") for send in self.sends):
                    raise RuntimeError("Native progress read escaped its accepted chat session")
                return {"tasks": []} if method == "tasks.list" else {"card": None}
