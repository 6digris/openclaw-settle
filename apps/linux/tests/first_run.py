...
def exercise(app, Atspi, GLib, *, remote_only, local_start_failure, inline_fixture, binary, gateway_switch):
    ...
    if local_start_failure:
        ...
        setup = "browser extension setup --action install --json --browser-profile chrome --wait-ms 1000"
    ...
    with tempfile.TemporaryDirectory(prefix="openclaw-first-run-") as directory:
        ...
        if args.local_start_failure:
            ...
            cli.write_text(
                ...
                "elif command == 'browser extension setup --action install --json --browser-profile chrome --wait-ms 1000':\n"
                "    print(json.dumps({'action': 'install', 'target': {'kind': 'local-host', 'platform': 'linux', 'hostname': 'fixture', 'profile': 'chrome', 'relayPort': 18799}, 'phase': 'needs_browser_action', 'reason': 'extension_missing', 'installation': {'nativeHostRegistered': True, 'installRequested': False, 'discoveredProfiles': 0, 'awaitingApproval': False, 'automaticBootstrapSupported': True}, 'connection': {'state': 'not_checked'}, 'nextAction': 'install_from_store'}))\n"
