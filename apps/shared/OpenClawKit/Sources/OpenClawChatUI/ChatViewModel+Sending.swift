...
extension OpenClawChatViewModel {
    ...
    private struct SendDraft {
        enum Source {
            case composer(replyTarget: OpenClawChatReplyTarget?, revision: UInt64, sessionKey: String)
        ...
        var outgoingMessageText: String {
            guard case let .composer(replyTarget?, _, _) = self.source else { return self.messageText }
    ...
    private func captureSendDraft() -> SendDraft? {
        ...
        return SendDraft(
            ...
            source: .composer(
                ...
                sessionKey: self.composerSessionKey(for: sessionKey)))
    }
    ...
    private func validateSendDraft(_ draft: SendDraft) async -> Bool {
        ...
        if case let .composer(_, revision, composerSessionKey) = draft.source,
    ...
    private func prepareLiveRoute(for draft: SendDraft) async -> OpenClawChatSubmissionOutcome? {
        guard case let .composer(_, revision, _) = draft.source else {
    ...
    {
        // Acceptance clears only the captured composer revision, even after
        // navigation. Presentation still belongs to the current route below.
    ...
    private func finishAcceptedComposerSend(_ draft: SendDraft) {
        guard case let .composer(replyTarget, revision, composerSessionKey) = draft.source else { return }
        self.recordSuccessfulInput(
            draft.trimmed,
