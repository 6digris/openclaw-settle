...
public struct OpenClawChatNativeActionGateway: Sendable {
    ...
    private let onProfileObservation: @Sendable (GatewayProfileBindingObservation) -> Void
        ...
        onProfileObservation: @escaping @Sendable (GatewayProfileBindingObservation) -> Void = { _ in })
    {
        ...
        self.request = { operation, expectedProfileID in
            do {
                return try await request(operation, expectedProfileID)
            } catch {
                if let observation = GatewayProfileBindingObservation.rejection(
                    from: error, expectedProfileID: expectedProfileID)
                {
                    onProfileObservation(observation)
                }
                throw error
            }
        }
        self.onProfileObservation = onProfileObservation
    ...
    public func owner(expected: OpenClawNativeOwnerRef? = nil) async throws -> OpenClawNativeOwnerRef {
        ...
        guard !profile.id.isEmpty, await self.isCurrent() else {
            throw OpenClawNativeActionError("The selected account changed. Select the session again.")
        }
        // Catalog discovery may succeed for a new account. Tell the captured
        // platform owner before returning it, even when no expected owner was supplied.
        self.onProfileObservation(.verified(profileID: profile.id))
        guard expected == nil || expected == owner else {
            throw OpenClawNativeActionError("The selected account changed. Select the session again.")
        }
        return owner
    }
