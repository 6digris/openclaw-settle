...
OpenClaw's GitHub runner-registration bucket reported 20,000 self-hosted
...
a new bucket. This September 20, 2026 reading comes from a pooled reader; its
unused quota does not prove organization-wide usage or available runner capacity.
...
target below about 60% of the live bucket. With the current 20,000-registration
bucket, that means a 12,000-registration operating target, leaving headroom for
concurrent repositories, retries, and burst overlap.

### Planning tiers

The workflow passes an explicit `fast` or `standard` tier to the Node planner.
`fast` applies only to `main` pushes in `openclaw/openclaw` and pull requests in
that repository whose head is the same repository, whose `head.repo.fork` is
false, and whose author association is `OWNER`, `MEMBER`, or `COLLABORATOR`.
Everything else uses `standard`, including fork PRs, `CONTRIBUTOR` authors,
manual dispatches, and non-canonical repositories. Concealed organization
membership can report `CONTRIBUTOR`; verify the actual association before
claiming a maintainer PR exercised `fast`. The planner never infers trust from
tokens. Existing backend expressions, runner classes, caches, worker limits,
...
budget is aggregate test work, not a promised job wall. Indivisible files and
groups retain their honest estimates even above the fast budget. The replay
still includes a 609-second plugin envelope and a 595-second CLI group. Fast
hybrid packing applies observed runtime floors before combining readers, so a
late floor cannot turn several smaller groups into an avoidable long job. The
remaining floors prevent a claim that
the eight-to-ten-minute workflow objective has been achieved merely by packing
more jobs.

There is no documented account-wide Linux concurrency ceiling. The six-run
September 20 main sample peaked at 37 simultaneous 32-class jobs in
[run 35518231054](https://github.com/openclaw/openclaw/actions/runs/35518231054):
36 Node jobs plus real-Gateway E2E. Fast plans therefore use a separate Node
...
wall, and waits per class; separate matrix admission and dependency waits from
runner assignment. A median provider assignment wait above 60 seconds requires
lowering the fast concurrency cap and measuring again.

For replay estimates, price Blacksmith Linux 32/16/8-class job minutes at
$0.064/$0.032/$0.016; the 4-class estimate uses the proportional $0.008 rate.
Include checkout/setup, required runtime preparation, and completion overhead
once per emitted job. Public-repository GitHub-hosted Linux minutes add no
Blacksmith charge. These are list-rate estimates without invoice rounding or
discounts. Nondist Node jobs depend on preflight, so the separate artifact build
overlaps them; do not add its whole duration to every Node estimate. Report
unchanged non-Node costs separately when projecting the full run.
...
Small precise PR changes use a focused Node plan. Broad, deleted or unknown changes retain compact core plus the affected plugin fallback; canonical pushes use the integration compact. Standard-tier compact plans are capped at 90 rows, and their plugin fallback packing is capped at 50. The standard final Node matrix also enforces 70 push rows or 130 PR rows, including precise plans; fast uses the limits above. Missing changed paths, missing current planner capabilities and planner errors fail preflight instead of emitting an incomplete successful matrix. Approved historical dispatches retain their full named plans. Count every emitted matrix row and nonmatrix job, including all six Android rows despite its two-job concurrency cap.
...
Complete standard-tier hybrid main and pull-request plans retain their existing
jobs and runner allocations while admitting measured runtime groups within
440 seconds, including the existing 100-second build allowance. Fast uses
250 seconds including that same allowance. The standard budget reserves 40 seconds of the
