...
Before adding a plugin job, the shared packer can exchange complete envelopes between
existing jobs. Both replacement jobs must still satisfy the same runner, preparation,
and 240-second limits. This preserves every test process and native shard while
reducing unused capacity for broad PR changes; it adds no runners or test stripes.
