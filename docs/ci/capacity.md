...
For canonical main pushes and trusted PRs selecting the two fixed Windows lanes, the separate `hybrid_hosted_checks` decision can add up to five measured check rows after that original admission, only while the combined total stays at or below 45 and the [hosted assignment guard](/ci/runners#hybrid-hosted-assignment-guard) reports fresh healthy evidence. The rows are dependencies, both core test-type stripes, extension package boundaries, and runtime topology architecture. Other PRs lack measured headroom and retain their 16-class routes; unhealthy or unavailable assignment evidence also retains those routes for that run. The original optional UI/security admission, 40-row base threshold, and 45-row total limit are unchanged.

No test inventory or worker budget changes. Control UI E2E shards, QA Smoke, real-Gateway checks, Android, builds, lint, and central test types retain their existing routes on eligible first attempts. Security now waits for preflight's decision; it still runs after a preflight failure unless the workflow was canceled. This exchanges parallel security startup for a complete hosted budget before runner allocation. Hosted assignment delay and ordinary cache setup can offset the compute saving, so compare the first exact-head run's hosted/Blacksmith row counts and queue times before widening the offload.
