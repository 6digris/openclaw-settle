...
The separate `hybrid_hosted_checks` decision can add up to five measured check rows after that original admission, only while the combined total stays at or below 45 and the [hosted assignment guard](/ci/runners#hybrid-hosted-assignment-guard) reports fresh healthy evidence. The rows are dependencies, both core test-type stripes, extension package boundaries, and runtime topology architecture. Unhealthy or unavailable evidence retains their 16-class routes for that run. The original optional UI/security admission, 40-row base threshold, and 45-row total limit are unchanged.
