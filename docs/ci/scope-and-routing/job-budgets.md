...
has 524,288 characters for every matrix together. Grouped and standalone Node
rows list their test files explicitly. The manifest projects the runner's group
contract, wrapping a standalone file list as one group, then uses gzip+base64
(`groups_gzip_base64`) when the target contains the codec. This preserves each
row and process envelope. Targets without that capability retain their original
standalone fields or projected legacy `groups` JSON. Workflow tests keep PR,
manual release-gate, and full-manual output under half of the cap. The smaller
