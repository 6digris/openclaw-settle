...
Groups from another agent never appear in the selected agent’s catalog, even with **Never** selected. Changing or clearing a filter never changes the saved preference. Populated groups stay visible even when collapsed, and hidden custom groups remain available in **Move to group**. Choose **Never** to recover their headers as drag targets. Catalog sections and empty agent groups in team mode retain their existing behavior. On narrow screens, the three choices open in the same menu with a **Back** action instead of a flyout.
...
**New group** from the sidebar, chat header, or Sessions page keeps the original session selection and its agent while the dialog is open and the group is being saved. Team-mode row menus use the clicked session’s agent, not the foreground conversation. Switching agents or replacing the connection retires open group actions rather than applying them to a same-named group in the new context. A group’s New Session link and preloaded defaults retain their requested agent. Before importing old browser-local group names, the browser durably binds one import receipt to the original Gateway, signed-in user, and configured default agent. Retries reuse that receipt: they append only names not already consumed, never resurrect deleted imported groups, and never copy pending names into a different destination. Names added locally during an import remain pending until acknowledged. Browsers without durable storage or cross-tab locking retain the source without sending it. Pending or failed imports do not block the current agent’s canonical group catalog; ambiguous old boot and roster group snapshots are not reused as an agent catalog. A deleted or replaced session is not moved; an error is shown and the new group remains available. For a sidebar multi-selection, sessions that still exist can move even if another target fails. Paging a selected session out of the visible list does not cancel its move.
