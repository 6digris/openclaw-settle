...
- In the Control UI, assistant images and attachments appear in message order, between their surrounding paragraphs. Consecutive images share a horizontal row; swipe, scroll, or focus the row and use the arrow keys to browse an overflowing set. Edge fades show which direction has more images. Text and other attachments separate rows, so before/after labels stay beside the corresponding images.
