...
| `onModelCatalog`   | No       | `boolean`                                            | Include this enabled plugin's declared agent harnesses in model catalog acquisition, even when no model selects them.                                                                       |
...
- Model catalog preparation includes enabled owners with `activation.onModelCatalog: true`. Explicit catalog acquisition calls `loadModelCatalog` for their `onAgentHarnesses` runtimes. The harness returns only available models; missing local installations return an empty list without downloading software. Repeated picker reads reuse the acquired inventory until catalog refresh or plugin reload.
