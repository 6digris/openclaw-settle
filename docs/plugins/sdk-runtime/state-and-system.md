...
<AccordionGroup>
  ...
  <Accordion title="api.runtime.state">
    ...
    Blob stores also provide optional `lookupInfo(key)`, which returns the same metadata, byte length, creation time, and expiry as `lookup` without loading the payload bytes. It preserves exact-key scoping, expiry, metadata decoding, and `lookup` errors, and does not create an absent database. A metadata read and a later payload read are separate observations: retain live reads and owner checks before using the payload. Diffs and Memory Wiki support the published 2026.9.4 host contract, so they use `store.lookupInfo ? await store.lookupInfo(key) : await store.lookup(key)` until their minimum host provides the capability. Fall back only when the method is absent, never after a failed metadata read.
