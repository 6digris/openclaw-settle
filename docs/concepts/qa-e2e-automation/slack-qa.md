...
The minimal driver manifest below does not grant those optional capabilities.
A missing scope blocks its operation and full lifecycle proof, not basic text
flows. Ask the app owner to provision an appropriately scoped QA app; the runner
does not grant permissions or rotate credentials to find a passing result.
