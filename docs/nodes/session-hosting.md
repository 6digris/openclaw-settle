...
application worker or node host crashes. Update and restart the node host to
receive this protection; installing a new worker bundle from the Gateway alone
does not update the node's supervisor. Recovery keeps capacity occupied while
the previous owner finishes stopping its commands.
