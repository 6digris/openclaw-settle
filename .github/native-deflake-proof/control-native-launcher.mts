  ...
  try {
    ...
    const run = async (bin: string, commandArgs: string[], timeoutMs?: number) => {
      ...
      try {
        const code = await runManagedCommand({
          ...
          timeoutMs: timeoutMs ?? Number(env.OPENCLAW_NATIVE_PROOF_TIMEOUT_MS ?? 180000),
        });
        canRemove = true;
        return code;
      } catch (error) {
        // Managed timeout is reported only after process-group and output drainage.
        if (error && typeof error === "object" && "code" in error && error.code === "ETIMEDOUT") {
          canRemove = true;
          console.error("[native-clock-proof]", error);
          return 124;
        }
        throw error;
      }
    ...
    try {
      ...
      if (fs.existsSync(eventStreamPath)) {
        fs.copyFileSync(
          eventStreamPath,
          path.join(env.RUNNER_TEMP, `clock-proof-${Date.now()}-swift-events.jsonl`),
        );
      }
